    ...
    namespace Species
{
    ...
    public class PKMDS
    {
        ...
        [DllImport(PKMDS_WIN32_DLL, CallingConvention = CallingConvention.Cdecl)]
        public static extern int GetTrainerTID_FromSav(string savefile);

        [DllImport(PKMDS_WIN32_DLL, CallingConvention = CallingConvention.Cdecl)]
        public static extern int GetTrainerSID_FromSav(string savefile);
        ...
        [DllImport(PKMDS_WIN32_DLL, CallingConvention = CallingConvention.Cdecl)]
        private static extern void GetPKMData_INTERNAL([In][Out] Pokemon pokemon, string savefile, int box, int slot);

        public static void GetPKMData([In][Out] ref Pokemon pokemon, string savefile, int box, int slot)
        {
            Pokemon pkm = new Pokemon();
            int size = Marshal.SizeOf(typeof(Pokemon));
            IntPtr pkmptr = Marshal.AllocHGlobal(size);
            Marshal.StructureToPtr(pkm, pkmptr, false);
            GetPKMData_INTERNAL(pkm, savefile, box, slot);
            pokemon = pkm;
